    ...
    return (
        <div className={` relative w-full  h-screen overflow-hidden flex flex-col  justify-center items-center md:flex-row `}>
                ...
                <div className={`w-10/12 sm:w-8/12 h-[75%] min-h-[300px] flex flex-col justify-center items-center `}>
                    ...
                    <span className={`w-[120%] flex justify-center text-xs leading-customLH text-center `}>کد ارسال شده به شماره  <span className='flex font-semibold text-xs'><Link href='/phonenumberlogin'><a className={` h-full flex  text-xs mx-2 text-Base`}>  {phoneNumber}<Pencil className='mx-2 ' color='green'/></a></Link></span>را وارد کنید </span>
                    ...
                    {Array.from({ length: 4}, (_, index) => (
                            ...
                            className={`w-[40px] h-[40px] text-center mx-[5px] rounded-md text-Base focus:outline-none bg-[#E6F6F4]  border border-Base `}
                            onInput={(e) => {
                    ...
                    {showButton&&<button className={`border-none text-sm mt-[25px] mb-[10px] text-Base font-semibold`} onClick={resetTimer}><span className='pl-[5px]  text-[12px] font-semibold text-gray-600'>کد دریافت نشده؟</span>ارسال مجدد</button>}
                    {/* <input ref={inputReference} className={`col-10  ${styles.codePlace}`} onChange={(ev)=>setCode(ev.target.value)} type='text'  placeholder='کد را وارد نمایید' /> */}
                       <button className={ `w-10/12 select-none border-none bg-Base text-white text-sm font-semibold rounded-[10px] h-[40px] mt-[20px] `} onClick={clickHandler}> تایید</button>
                        <div className={'text-red-500 text-xs font-semibold h-[20px] mt-2  '}>{errCode}</div>
                        <Link href={'/userpasslogin'}><a className={'mt-[20px] text-sm font-semibold'}>ورود با <small className='text-Base text-sm'>نام کاربری و رمز عبور </small></a></Link>
                    </div>
                </div>
                {/* Left Parent------------------------------- */}
